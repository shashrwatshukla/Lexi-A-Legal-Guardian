...
void main() {
  ...
  float heightFade = smoothstep(0.4, 1.0, uv.y);
  
  vec3 auroraColor = intensity * rampColor;
  
  fragColor = vec4(auroraColor * auroraAlpha * heightFade, auroraAlpha * heightFade);
}
`;
...
export default function Aurora(props) {
  ...
  useEffect(() => {
    ...
    const update = t => {
      ...
      // ✅ Skip rendering if not visible or less than 16ms passed
      if (!isVisibleRef.current || (t - lastTime < 16)) {
        return;
      }
      ...
      program.uniforms.uTime.value = time * (propsRef.current.speed || speed) * 0.08; // ✅ Slower
